...
1) **CR‑First** — Every change merges via a Change Request with a **Story Preview** and **DoD Signals**.
...
- **Roles** — Product Owner; **Delivery Lead** (Scrum Master equivalent + authority over signals/lease); Developers (human + agent).
...
- Require **`tests-ci`** and **`spec-verify`** signals.
...
- **DoD Signals (what must be green)** → `../handbook/cr-gates.md`
